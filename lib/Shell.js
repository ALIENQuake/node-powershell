...
class Shell extends EventEmitter {
    constructor(options = {}) {
        ...
        let psProc = ''
        if (fs.existsSync('./powershell/') || fs.existsSync('./powershell-preview/')) {
            if (options.pwshPrev) {
                if (fs.existsSync('./powershell-preview/' + 'pwsh-preview')) {
                    let tempPSProc = (process.cwd() + '/powershellh-preview/' + 'pwsh-preview').replace(/\\/g, "/") // Windows dosen't mind '/'
                    psProc = tempPSProc;
                } else {
                    // this covers Windows Powershell Core Preview still having pwsh as executable basename
                    let tempPSProc = (process.cwd() + '/powershell-preview/' + 'pwsh').replace(/\\/g, "/") // Windows dosen't mind '/'
                    psProc = tempPSProc;
                }
            } else {
                // even if folder exist, don't use powershell-preview without explict option
                let tempPSProc = (process.cwd() + '/powershell/' + 'pwsh').replace(/\\/g, "/") // Windows dosen't mind '/'
                psProc = tempPSProc;
            }
        } else {
            if (isWin) {
                if (options.pwsh || options.pwshPrev) {
                    psProc = options.pwshPrev ? 'pwsh-preview' : 'pwsh'
                } else {
                    psProc = 'powershell'
                }
            } else {
        ...
        if ('debugMsg' in options) {
            logger.warn(`debugMsg will be deprecated soon. use verbose instead. https://rannn505.gitbook.io/node-powershell/shell/initialize`);
